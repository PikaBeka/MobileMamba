...
data.root = '/l/users/jose.viera/datasets/ILSVRC/Data/CLS-LOC'
...
data.test_transforms = [
    dict(type='Resize', size=int(size / 0.875),
         interpolation=_F.InterpolationMode.BICUBIC),
    dict(type='CenterCrop', size=size),
    dict(type='ToTensor'),
    dict(type='Normalize', mean=_IMAGENET_DEFAULT_MEAN,
         std=_IMAGENET_DEFAULT_STD, inplace=True),
]

# =========> model <=================================
model = _Namespace()
model.name = 'MobileMamba_S6'
model.model_kwargs = dict(pretrained=False, checkpoint_path='',
                          ema=False, strict=True, num_classes=data.nb_classes)

# =========> optimizer <=================================
optim = _Namespace()
optim.lr = lr
optim.optim_kwargs = dict(name='adamw', betas=(
    0.9, 0.999), eps=1e-8, weight_decay=weight_decay, amsgrad=False)
# =========> trainer <=================================
trainer = _Namespace()
...
trainer.data.num_workers_per_gpu = 6
...
trainer.scale_kwargs = dict(n_scale=0, base_h=size, base_w=size, min_h=160,
                            max_h=320, min_w=160, max_w=320, check_scale_div_factor=32)
...
tea_model.model_kwargs = dict(pretrained=False, checkpoint_path='pretrained/tresnet_l_v2_83_9.pth',
                              ema=False, strict=True, num_classes=data.nb_classes)

loss = _Namespace()
loss.loss_terms = [
    dict(type='SoftTargetCE', name='CE', lam=1.0, fp32=True) if trainer.mixup_kwargs['prob'] > 0 else dict(
        type='LabelSmoothingCE', name='CE', lam=1.0, smoothing=trainer.mixup_kwargs['label_smoothing']),
